...
@METRICS.register_module()
class StrictMultiLabelMetric(BaseMetric):
    r"""A collection of precision, recall, f1-score and support for
    multi-label tasks, where only fully correct predictions are considered correct.
    ...
    Args:
        ...
        prefix (str, optional): The prefix that will be added in the metric
            ...
            will be used instead. Defaults to None.
    """
    default_prefix: Optional[str] = 'strict-multi-label'
    ...
    def __init__(self,
        ...
        for item in items:
            assert item in ['precision', 'recall', 'f1-score', 'support'], \
                f'The metric {item} is not supported by `StrictMultiLabelMetric`,' \
                ' please choose from "precision", "recall", "f1-score" and ' \
                '"support".'
        self.items = tuple(items)
    ...
    def calculate(
        ...
        # Only consider samples where all labels are correct
        correct = (pos_inds == target).all(dim=1).float()
        tp = correct.sum()
        fp = (pos_inds.sum(dim=1) > 0).float().sum() - tp
        fn = (target.sum(dim=1) > 0).float().sum() - tp
